...
use crate::platform::mlock;
#[cfg(any(windows, target_arch = "wasm32"))]
use crate::platform::munlock;
#[cfg(not(any(windows, target_arch = "wasm32")))]
use crate::platform;
...
/// Allocate zeroed, mlocked memory.
///
/// On Unix, uses `mmap`/`mlock` directly (bypassing the global allocator)
/// so that `dealloc_locked` can return pages to the OS immediately via

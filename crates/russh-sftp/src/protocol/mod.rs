...
impl TryFrom<&mut Bytes> for Packet {
    ...
    fn try_from(bytes: &mut Bytes) -> Result<Self, Self::Error> {
        ...
        let request = match r#type {
            ...
            SSH_FXP_DATA => {
                // Bypass serde for the hot path: parse id + data with zero-copy split.
                let id = bytes.try_get_u32()?;
                let data_len = bytes.try_get_u32()? as usize;
                if bytes.remaining() < data_len {
                    return Err(Error::BadMessage("SSH_FXP_DATA: not enough data".to_owned()));
                }
...
impl TryFrom<Packet> for Bytes {
    ...
    fn try_from(packet: Packet) -> Result<Self, Self::Error> {
        let (r#type, payload): (u8, Bytes) = match packet {
            ...
            Packet::Data(data) => {
                // Bypass serde for Data packets — serialize id + length-prefixed data directly.
                let mut payload = BytesMut::with_capacity(4 + 4 + data.data.len());
                payload.put_u32(data.id);
                payload.put_u32(data.data.len() as u32);
                payload.put_slice(&data.data);
                (SSH_FXP_DATA, payload.freeze())
            }
